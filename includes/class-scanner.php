...
class AJS_Scanner {
    ...
    public function run_full_scan(bool $auto_heal = false, bool $ai_deep_scan = false): array {
        ...
        if ($skip_uploads) {
            ...
            $this->scan_directory_for_scripts($uploads_path, $findings, $auto_heal, $skipped_paths);
        }

        // 2. Scan active theme & plugin root for webshell signatures
        $theme_dir = get_stylesheet_directory();
        if (is_dir($theme_dir)) {
            $this->scan_directory_for_signatures($theme_dir, $findings, $skipped_paths);
        }

        // 3. AI Deep Scan: Check published articles & pages for judol injections
        if ($ai_deep_scan && $this->ai && $this->ai->is_configured()) {
            $this->scan_posts_for_judol($findings, $auto_heal);
        }
        ...
        $this->scan_database_options($findings);

        // 6. WordPress.org Official Core Checksums Verification
        $this->scan_wordpress_core_checksums($findings);
        ...
        switch ($step) {
            ...
            case 'scan_uploads':
                ...
                if ($skip_uploads) {
                    ...
                    $this->scan_directory_for_scripts($uploads_path, $session_data['findings'], $auto_heal, $session_data['skipped_paths']);
            ...
            case 'scan_theme':
                ...
                if (is_dir($theme_dir)) {
                    ...
                    $this->scan_directory_for_signatures($theme_dir, $session_data['findings'], $session_data['skipped_paths']);
                    $found_here = count($session_data['findings']) - $before_count;
            ...
            case 'scan_ai':
                ...
                if ($this->ai && $this->ai->is_configured()) {
                    $before_count = count($session_data['findings']);
                    $this->scan_posts_for_judol($session_data['findings'], $auto_heal);
                    $found_here = count($session_data['findings']) - $before_count;

                    if ($found_here > 0) {
                        $logs[] = "AI Screening: Ditemukan {$found_here} artikel disusupi link/materi judi online!";
                    } else {
                        $logs[] = 'AI Screening: Sampel artikel terbaru terbitan bersih dari materi judi online.';
                    }
                } else {
            ...
            case 'scan_database':
                $logs = [];
                $before_count = count($session_data['findings']);
                $this->scan_database_options($session_data['findings']);
                $found_here = count($session_data['findings']) - $before_count;
            ...
            case 'scan_core':
                $logs = [];
                $before_count = count($session_data['findings']);
                $this->scan_wordpress_core_checksums($session_data['findings']);
                $found_here = count($session_data['findings']) - $before_count;
            ...
            case 'finalize':
                ...
                $logs = [
                    'Pemindaian menyeluruh berhasil diselesaikan!',
    ...
    public function scan_database_options(array &$findings): void {
        global $wpdb;
        $patterns = [
    ...
    public function scan_wordpress_core_checksums(array &$findings): void {
    ...
    private function scan_posts_for_judol(array &$findings, bool $auto_heal = false): void {
        $posts = get_posts([
            'post_type'      => ['post', 'page'],
            'post_status'    => 'publish',
            'posts_per_page' => 20,
        ...
        foreach ($posts as $p) {
            ...
            $ai_check = $this->ai->inspect_content($text_sample);

            if (!empty($ai_check['is_threat'])) {
                $healed = false;
                if ($auto_heal) {
    ...
    private function scan_directory_for_scripts(string $dir, array &$findings, bool $auto_heal, array &$skipped_paths = []): void {
        $excluded_rules = $this->get_excluded_paths();
        $skip_nfs       = (int)get_option('ajs_scan_skip_nfs', 1) === 1;
        $network_mounts = $skip_nfs ? $this->get_network_mounts() : [];
    ...
    private function scan_directory_for_signatures(string $dir, array &$findings, array &$skipped_paths = []): void {
        $excluded_rules = $this->get_excluded_paths();
        $skip_nfs       = (int)get_option('ajs_scan_skip_nfs', 1) === 1;
        $network_mounts = $skip_nfs ? $this->get_network_mounts() : [];
